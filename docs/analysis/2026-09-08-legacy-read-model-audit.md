...
However, the mutation path changes an existing entity's `type_id` when an upsert supplies a different `entityType` (`crates/memory-core/src/mutation.rs:615-635`). The core regression test proves `old` becomes `new` and observations persist (`crates/memory-core/src/graph.rs:2389-2409`):

```
cargo test -p memory-core test_upsert_entities -- --test-threads=1
# 1 passed
```
...
**Evidence.** Storage has a non-null creation timestamp: `observation.created_us` is declared in the primary schema (`crates/memory-core/src/graph.rs:455-464`) and insertion supplies `now_us()` (`crates/memory-core/src/mutation.rs:516-538`). This was already present in legacy commit `d6fe34b`.

The read model selects only `body` (`crates/memory-core/src/mutation.rs:309-329`) and serializes observations as `Vec<String>` (`crates/memory-core/src/types.rs:3-10`). `describe_entity` returns that same model. The new event log's `occurred_at_us` timestamps mutations, not individual observations (`crates/memory-core/src/events.rs:100-142`; `migrations/0001_change_events.sql:6-14`).
...
**Migration/API risk.** This is a schema and public-contract change. Use a new numbered startup migration (`ALTER TABLE observation ADD COLUMN occurred_us INTEGER`), not an edit to migration 1: migration ordering and checksum validation are enforced at `crates/memory-core/src/events.rs:38-82`. Changing `Entity.observations` directly from strings to objects would break MCP responses, event payloads and indexer content.

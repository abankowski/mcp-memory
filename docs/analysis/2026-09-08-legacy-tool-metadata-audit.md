...
**Naprawione.** `MutationRequest::DeleteRelations` iteruje wejście i dla każdej trójki wykonuje stały, parametryzowany `DELETE`; nie buduje listy `SELECT` ([mutation.rs](/Users/abankowski/Projects/abankowski/mcp-memory/crates/memory-core/src/mutation.rs:655)). Test pokrywa batch z fizycznym duplikatem i sprawdza licznik ([mutation_service.rs](/Users/abankowski/Projects/abankowski/mcp-memory/tests/mutation_service.rs:362)).

**Test regresji:** E2E MCP: trzy różne relacje w jednym `delete_relations`; `isError == false` i żadnej z nich nie zwraca `open_nodes`.

## 2. `describe_entity`

**Odziedziczone, reprodukcja statyczna.** Manifest obiecuje relacje incydentne, neighbours i degree ([tools.json](/Users/abankowski/Projects/abankowski/mcp-memory/tools.json:247)), ale implementacja jest wyłącznie `get_entity` ([graph.rs](/Users/abankowski/Projects/abankowski/mcp-memory/crates/memory-core/src/graph.rs:1559)), a handler serializuje ten `Entity` ([memory.rs](/Users/abankowski/Projects/abankowski/mcp-memory/src/actions/memory.rs:465)).
...
**Częściowo naprawione.** `create_relation` używa `INSERT ... WHERE NOT EXISTS` na `(from_id,to_id,type_id)` ([mutation.rs](/Users/abankowski/Projects/abankowski/mcp-memory/crates/memory-core/src/mutation.rs:563)); merge przekierowuje relacje przez tę funkcję ([mutation.rs](/Users/abankowski/Projects/abankowski/mcp-memory/crates/memory-core/src/mutation.rs:689)). To eliminuje repro dla nowych danych.

Schemat nie wymusza tej własności: `relation` ma zwykłe indeksy, bez `UNIQUE` ([graph.rs](/Users/abankowski/Projects/abankowski/mcp-memory/crates/memory-core/src/graph.rs:466)). Historyczne duplikaty przetrwają merge, a inna ścieżka SQL może je wprowadzić.
...
**Naprawione.** `apply_inner` otwiera `TxGuard` przed snapshotem, wykonuje całą mutację i zapisuje change events/outbox przed commitem ([mutation.rs](/Users/abankowski/Projects/abankowski/mcp-memory/crates/memory-core/src/mutation.rs:231)). Przenoszenie obserwacji, przekierowanie relacji i usunięcie source są jedną gałęzią `execute` ([mutation.rs](/Users/abankowski/Projects/abankowski/mcp-memory/crates/memory-core/src/mutation.rs:689)).
...
**Odziedziczone.** `observation` przechowuje tylko `entity_id`, `idx`, `body`, `created_us` ([graph.rs](/Users/abankowski/Projects/abankowski/mcp-memory/crates/memory-core/src/graph.rs:455)); merge kopiuje same teksty do target ([mutation.rs](/Users/abankowski/Projects/abankowski/mcp-memory/crates/memory-core/src/mutation.rs:693)). `change_event` ma payload zmiany, nie `source_entity` per obserwacja ([0001_change_events.sql](/Users/abankowski/Projects/abankowski/mcp-memory/migrations/0001_change_events.sql:6)).
...
**Zmienione częściowo.** `upsert_entities` aktualizuje `type_id` istniejącej encji ([mutation.rs](/Users/abankowski/Projects/abankowski/mcp-memory/crates/memory-core/src/mutation.rs:615)); jest na to test ([graph.rs](/Users/abankowski/Projects/abankowski/mcp-memory/crates/memory-core/src/graph.rs:2388)). Opis nadal mówi, że typ jest stosowany tylko przy tworzeniu ([tools.json](/Users/abankowski/Projects/abankowski/mcp-memory/tools.json:277)). Nie ma publicznego rename ani jawnego retype.
...
**Częściowo naprawione.** `created_us` istnieje w schemacie i jest ustawiane przez serwer ([graph.rs](/Users/abankowski/Projects/abankowski/mcp-memory/crates/memory-core/src/graph.rs:455), [mutation.rs](/Users/abankowski/Projects/abankowski/mcp-memory/crates/memory-core/src/mutation.rs:516)). Nie jest częścią odpowiedzi `Entity`, nie ma `occurred_at`, a `change_event.occurred_at_us` dotyczy mutacji, nie faktu ([0001_change_events.sql](/Users/abankowski/Projects/abankowski/mcp-memory/migrations/0001_change_events.sql:6)).
...
**Naprawione dla obecnego MCP.** `delete_entities`, `delete_observations`, `delete_relations` mają `readOnlyHint:false, destructiveHint:true` ([tools.json](/Users/abankowski/Projects/abankowski/mcp-memory/tools.json:71)), tak samo `merge_entities` ([tools.json](/Users/abankowski/Projects/abankowski/mcp-memory/tools.json:311)) i `vector_delete_embedding` ([vector_tools.json](/Users/abankowski/Projects/abankowski/mcp-memory/vector_tools.json:53)). Pozostałe mutatory są non-read-only, lecz non-destructive; czytelniki mają `readOnlyHint:true`.

Metadane są ładowane z JSON wbudowanego przez `include_str!` i zwracane w kolejności plików ([server.rs](/Users/abankowski/Projects/abankowski/mcp-memory/src/server.rs:563), [server.rs](/Users/abankowski/Projects/abankowski/mcp-memory/src/server.rs:595)). Zbiór zależy celowo od kategorii/feature'ów. Zastrzeżenie: flagi kategorii są globalnymi atomikami ustawianymi w konstruktorze ([server.rs](/Users/abankowski/Projects/abankowski/mcp-memory/src/server.rs:244)), więc dwa serwery o różnych konfiguracjach w jednym procesie nie izolują `tools/list`; CLI z jednym serwerem nie jest tym dotknięte.
